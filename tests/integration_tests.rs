...
fn binary_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("target/debug/search-sessions")
}
...
mod index_parsing {
    ...
    fn test_sessions_index_parsing() {
        ...
        assert_eq!(parsed["entries"][0]["summary"], "Discussing Kubernetes RBAC configuration");
    }
}
...
mod claude_session_parsing {
    ...
    fn test_claude_session_format() {
        ...
        let summary: serde_json::Value = serde_json::from_str(lines[0]).expect("Failed to parse summary");
        assert_eq!(summary["type"], "summary");
        
        // Parse user message
        let user_msg: serde_json::Value = serde_json::from_str(lines[1]).expect("Failed to parse user message");
        assert_eq!(user_msg["type"], "user");
        assert_eq!(user_msg["sessionId"], "test-session-1");
        
        // Parse assistant message
        let asst_msg: serde_json::Value = serde_json::from_str(lines[2]).expect("Failed to parse assistant message");
    ...
    fn test_claude_message_content_extraction() {
        ...
        let content_arr = message["content"].as_array().expect("Content should be array");
...
mod openclaw_session_parsing {
    ...
    fn test_openclaw_session_format() {
        ...
        let header: serde_json::Value = serde_json::from_str(lines[0]).expect("Failed to parse header");
        assert_eq!(header["type"], "session");
        assert_eq!(header["id"], "test-openclaw-1");
        assert_eq!(header["cwd"], "/home/user/projects/myapp");
        
        // Parse message
        let msg: serde_json::Value = serde_json::from_str(lines[1]).expect("Failed to parse message");
    ...
    fn test_openclaw_message_content_extraction() {
        ...
        let role = msg["message"]["role"].as_str().expect("Role should be string");
        assert_eq!(role, "user");
        
        let content_arr = msg["message"]["content"].as_array().expect("Content should be array");
        assert!(content_arr[0]["text"].as_str().unwrap().contains("security audit"));
    }
}
